...
Before changing links, the batch records affected page IDs in
`.llmwiki/review-embedding-intent.json`, associated with the exact candidate
snapshots. A reordered retry, or a retry of a retained subset, recovers that
...
invalid, oversized, or unwritable intent storage fails the operation and keeps
candidates for retry. A full or unreadable embedding retry queue likewise fails
batch finalization rather than forgetting unrecorded work. Repair storage or
free retry capacity, then retry the retained candidates. Keep their snapshots
unchanged until recovery finishes: editing or rejecting them does not erase
their intent. If snapshots were removed, restore them from backup to complete
recovery; do not delete the intent file merely to bypass a storage error.
