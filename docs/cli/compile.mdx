...
| `--verbose` | Print detailed per-step progress: source sizes, concept merge details, page char counts, semantic-index summary, and total compile time. Equivalent to setting `LLMWIKI_VERBOSE=1`. Quiet mode (`--json`) always wins over verbose. |
...
The same incremental logic applies to the semantic index. The local backend
updates only page and chunk vectors whose content changed. R2R ingests only new
or changed page documents and deletes documents for removed pages.
...
With the default local backend, compile sends page and chunk embeddings to the
provider in batches. The summary reports how many requests this took, for
example `Embeddings: 12/12 pages, 84/84 chunks embedded (3 batched requests)`.
Tune the request size with
[`LLMWIKI_EMBED_BATCH_SIZE`](/configuration/environment-variables#embeddings).

With [R2R selected](/configuration/semantic-backends), compile sends pre-chunked
page documents to R2R instead. R2R owns vector generation, persistence, and
indexed lookup. Set
[`LLMWIKI_EMBED_STRICT`](/configuration/environment-variables#embeddings) to
fail the run when either backend cannot refresh.

<Note>
  A semantic refresh failure is non-fatal by default. Compile prints a warning,
  preserves page writes, and retries pending page IDs on a later run. Strict
  mode changes the exit status; it does not roll back pages already written.
</Note>
...
.llmwiki/
  embeddings.json     local backend: page and chunk vectors
  r2r-index-*.json    R2R backend: IDs and freshness hashes, no vectors or text
  state.json          per-source content hashes and compile timestamps
```

Only the selected semantic backend is refreshed. Switching backends does not
delete the other backend's local state.
