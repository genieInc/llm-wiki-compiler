...
<Steps>
  <Step title="Semantic candidate retrieval">
    The default `.llmwiki/embeddings.json` backend scores chunk vectors in process. The optional R2R backend asks its namespace in R2R for indexed results. Both narrow the wiki to a small top-K under qualified page ids.
  </Step>
