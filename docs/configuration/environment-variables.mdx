...
| `LLMWIKI_EMBEDDINGS` | Enabled | Set to `off` (case-insensitive) to skip all embedding refreshes, including compile, review approval, import, and `query --save`. Existing embedding and pending files are left untouched. |
