...
| `R2R_NAMESPACE` | Required for R2R | Stable lowercase wiki identifier, unique within the R2R endpoint and project |
...
The environment-backed adapter lets R2R assign documents to the authenticated
user's default collection. These variables configure one process-wide endpoint,
namespace, and credential binding. They are suitable for the CLI, a
tenant-specific child process, or a single-tenant service. An in-process
multi-tenant SDK host should pass an explicit
