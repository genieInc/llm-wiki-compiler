...
description: "Complete reference for all llmwiki environment variables: provider and semantic backend selection, API keys, model overrides, timeouts, output language, and debug flags."
---
...
When the local semantic backend is active and you set
`LLMWIKI_EMBEDDING_PROVIDER`, the named provider's own credential is required:
`VOYAGE_API_KEY` for `anthropic` or `claude-agent`, `OPENAI_API_KEY` (or
`OPENAI_EMBEDDINGS_API_KEY`) for `openai`, and no key for `ollama`. Setting
`OPENAI_EMBEDDINGS_BASE_URL` marks a self-hosted OpenAI-compatible endpoint that
needs no key, so `OPENAI_API_KEY` is not required in that case. This check does
not run when `LLMWIKI_SEMANTIC_BACKEND=r2r`, because R2R owns vector generation.
...
The local `.llmwiki/embeddings.json` index remains the default. Select R2R
explicitly when you want an external vector database and indexed retrieval.

| Variable | Default | Description |
|---|---|---|
| `LLMWIKI_SEMANTIC_BACKEND` | `local` | Semantic index implementation. One of `local` or `r2r` |
| `R2R_BASE_URL` | `http://localhost:7272` | R2R server URL before `/v3`. Plain HTTP is accepted only for loopback hosts unless explicitly allowed |
| `R2R_COLLECTION_ID` | Required for R2R | UUID of a collection dedicated to this wiki |
| `R2R_NAMESPACE` | Required for R2R | Stable lowercase wiki identifier used as an additional metadata filter inside the collection |
| `R2R_API_KEY` | *(unset)* | API key sent as `x-api-key` |
| `R2R_ACCESS_TOKEN` | *(unset)* | Bearer token. Mutually exclusive with `R2R_API_KEY` |
| `R2R_PROJECT_NAME` | *(unset)* | Optional R2R project sent as `x-project-name` |
| `R2R_SEARCH_MODE` | `basic` | R2R retrieval preset: `basic`, `advanced`, or `custom` |
| `R2R_TIMEOUT_MS` | `600000` | Per-request timeout in milliseconds; must be a positive integer |
| `R2R_INGEST_CONCURRENCY` | `5` | Concurrent R2R ingest/delete operations; positive values above `50` are capped |
| `R2R_MAX_RETRIES` | `3` | Retry count for transient R2R failures; non-negative values above `10` are capped |
| `R2R_RETRY_BASE_DELAY_MS` | `250` | Initial exponential retry delay in milliseconds; values above `10000` are capped |
| `R2R_FULL_SYNC_INTERVAL_MS` | `86400000` | Interval in milliseconds between full local audits; ordinary refreshes read only changed IDs |
| `R2R_ALLOW_INSECURE_HTTP` | *(unset)* | Set to `1`, `true`, `yes`, or `on` to permit a non-loopback HTTP endpoint |

When R2R is selected, R2R generates and stores vectors. Local embedding-provider
settings are not used, but chat-provider credentials are still required for
commands that generate text. See [Semantic Backends](/configuration/semantic-backends)
for setup, data exposure, manifest, recovery, and switching behavior.

These R2R variables configure one process-wide binding. They are suitable for
the CLI, a tenant-specific child process, or a single-tenant service. An
in-process multi-tenant SDK host should pass an explicit
`createR2RSemanticBackend` instance to each Wiki, or use its root-aware async
resolver; it must not switch tenants by mutating `process.env` concurrently.
See [Multi-tenant SDK hosts](/configuration/semantic-backends#multi-tenant-sdk-hosts).

---

## Embeddings

With the default local semantic backend, `llmwiki compile` embeds pages and
chunks in provider-native batches. `LLMWIKI_EMBED_STRICT` also applies to R2R
...
<Note>
  Batch size applies only to the local backend. R2R owns its embedding batches.
  Selecting R2R leaves an existing `.llmwiki/embeddings.json` file untouched.
</Note>
...
# Optional: use R2R instead of .llmwiki/embeddings.json
# LLMWIKI_SEMANTIC_BACKEND=r2r
# R2R_BASE_URL=http://localhost:7272
# R2R_COLLECTION_ID=123e4567-e89b-42d3-a456-426614174000
# R2R_NAMESPACE=my-team-wiki
