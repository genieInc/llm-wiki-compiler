...
| `LLMWIKI_EMBEDDINGS` | Enabled | Set to `off` (case-insensitive) to skip embedding refreshes during compile, review approval, and import. Existing embedding and pending files are left untouched. |
